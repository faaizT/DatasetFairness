...
def main(args):
    ...
    if form == "logistic":
        ...
        def floss(outputs, sens_attr):
            return lam_fair/32 * torch.abs(-outputs[sens_attr].sum()/Pmale + outputs[~sens_attr].sum()/Pfem)
    elif form == "weighting":
        def floss(outputs, sens_attr):
            return -lam_fair/32 * (F.logsigmoid(outputs[sens_attr]).sum()/Pmale - F.logsigmoid(outputs[~sens_attr]).sum()/Pfem)
    ...
    torch.save(model.state_dict(), f"{args.model_dir}/model_{form}_{lam_fair}_{args.seed}_eop.pth")
