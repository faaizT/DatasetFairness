...
class CelebAClassifierWithIntermediateOutput(nn.Module):
    def __init__(self, hidden_layer_size=128):
        super(CelebAClassifierWithIntermediateOutput, self).__init__()
    ...
    def forward(self, x):
        ...
        x = x.view(x.size(0), -1)
        prev_output = F.relu(self.fc1(x))
        x = torch.sigmoid(
            self.fc2(prev_output)
        )  # sigmoid activation for binary classification
        return x, prev_output
...
def pretrain_classifier(classifier, optimizer_clf, train_loader, epochs, device):
    ...
    for epoch in range(epochs):
        ...
        for i, data in enumerate(train_loader):
            ...
            if i % 200 == 0:
                metrics = {"i": i, "classifier loss": epoch_loss / (i + 1)}
                wandb.log(metrics)
                print(metrics)

        wandb.log({"Epoch": epoch + 1, "classifier Loss": epoch_loss / len(train_loader)})
        ...
        for i, data in enumerate(train_loader):
            ...
            if i % 200 == 0:
                metrics = {
                    "i": i,
                    "Loss": epoch_loss / (i + 1),
                    "percentage": i * labels.shape[0] / (len(train_loader)),
                }
                wandb.log(metrics)
                print(metrics)

        wandb.log({"Epoch": epoch + 1, "adv Loss": epoch_loss / len(train_loader)})
        print({"Epoch": epoch + 1, "adv Loss": epoch_loss / len(train_loader)})
    return adversary
...
def main(args):
    ...
    if config.input_dim == 512:
        ...
        if config.input_dim == 64 * 64 * 3:
            model = CelebAClassifierWithIntermediateOutput(hidden_layer_size=64).to(
                device
            )
    ...
    adversary = Adversary(
        classifier_hidden_dim=hidden_layer_size, fairness=config.fairness
    ).to(device)
    ...
    if config.input_dim == 512:
        ...
        model = pretrain_classifier(
            model, optimizer_clf, train_loader, epochs=100, device=device
        )
        for param in adversary.parameters():
    ...
    if config.input_dim == 12288:
        epochs_adversary = 100
    else:
        epochs_adversary = 1
